...
class UserValidationInterpreter[F[_] : Monad](userDAO: UserDAOAlgebra[F]) extends UserValidationAlgebra[F] {
  ...
  override def doesNotExist(maybeUserName: Option[String], maybeEmail: Option[String]): F[ValidatedNel[UserValidationError, Unit]] = {
    val validateUserNameF: F[ValidatedNel[UserValidationError, Unit]] = maybeUserName.map(validateUserNameUniques).getOrElse(Monad[F].pure(Valid(())))
    val validateEmailF: F[ValidatedNel[UserValidationError, Unit]] = maybeEmail.map(validateEmailUniques).getOrElse(Monad[F].pure(Valid(())))
